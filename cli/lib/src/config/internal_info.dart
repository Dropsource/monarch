...
class InternalInfo {
  ...
  final String previewApiVersion;
  ...
  const InternalInfo({
    ...
    required this.previewApiVersion,
  });
}
...
Future<InternalInfo> readInternalFiles(MonarchBinaries monarchBinaries) async {
  var internal = InternalInfo(
    ...
    previewApiVersion:
        await _readInternalFile('preview_api_version.txt', monarchBinaries),
  );
  ...
  _logger.config('monarch binaries:'
      ...
      ' preview_api_version=${internal.previewApiVersion}');
