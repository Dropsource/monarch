...
class PackageConfigHelperTester extends PackageConfigHelper {
  PackageConfigHelperTester(
      {required this.packageConfigContents}): super(Directory.current);
  ...
  group('PackageConfigHelper', () {
    group('on windows', () {
      test('parses file uri to windows path', () async {
        final helper = PackageConfigHelperTester(packageConfigContents: someWindowsPackageConfig);
        await helper.setUp();
        ...
        expect(helper.getPackageRootPath('args'), r'C:\Users\john\flutter\.pub-cache\hosted\pub.dartlang.org\args-2.3.0');
        expect(helper.getPackageRootPath('convert'), r'C:\Users\john\flutter\.pub-cache\hosted\pub.dartlang.org\convert-3.0.1');
        expect(helper.getPackageRootPath('flutter'), r'C:\Users\john\flutter\packages\flutter');
      });
    }, testOn: 'windows');

    group('on macos', () {
      test('parses file uri to macos path', () async {
        final helper = PackageConfigHelperTester(packageConfigContents: someMacosPackageConfig);
        await helper.setUp();
        ...
        expect(helper.getPackageRootPath('args'), '/Users/john/flutter/.pub-cache/hosted/pub.dartlang.org/args-2.3.0');
        expect(helper.getPackageRootPath('convert'), '/Users/john/flutter/.pub-cache/hosted/pub.dartlang.org/convert-3.0.1');
        expect(helper.getPackageRootPath('flutter'), '/Users/john/flutter/packages/flutter');
      });
    }, testOn: 'mac-os');
  });
  ...
  group('FlutterPackageHelper', () {
    group('on windows', () {
      ...
      test('does not end with trailing dirs', () {
        final helper =
            FlutterPackageHelper(r'C:\foo\bar\packages\floating');
        expect(helper.endsWithTrailingDirs, isFalse);
      });
      test('gets flutter executable path', () {
        final path =
            r'C:\Users\foo\development\flutter-sdk\packages\flutter';
    ...
    group('on macos', () {
      ...
      test('gets flutter executable path', () {
        final path =
            r'/Users/foo/development/flutter-sdk/packages/flutter';
        final helper = FlutterPackageHelper(path);
        expect(helper.getExecutablePath(),
