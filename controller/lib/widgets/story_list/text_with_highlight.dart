...
class TextWithHighlight extends StatelessWidget{
  ...
  const TextWithHighlight({Key? key, required this.text, this.highlightedText = ''}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return RichText(
        text: TextSpan(
            children: _buildHighlighters(context)));
  }
  ...
  List<TextSpan> _buildHighlighters(BuildContext context) {
    ...
    if (highlightedText.isEmpty) {
      ...
      final highlighterTextSpans = [highlightedText]
        ...
        if (highlightedText.isEmpty) {
          ...
          if (splitTexts.length > 1) {
            splitTextSpans.add(TextSpan(
                text: splitTexts.first,
                style: textStyle));
            splitTextSpans.add(TextSpan(
      ...
      highlighterTextSpans.add(TextSpan(
          text: originalText, style: textStyle));
